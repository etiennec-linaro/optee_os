...
/*
 * TODO: a single page is enought
 * Only 1 non-secure RX message buffer is needed. The are no other secure
 * world (armv7-A). Core does not need a non-secure TX buffer, SPCI pushes
 * stright the message to the target mailbox message buffer.
 */
#define CFG_SPCI_MSG_BUF_SIZE	(4 * SMALL_PAGE_SIZE)
#endif
...
struct msg_buf_list_exchange_hdr {
	uint32_t signature;
	uint16_t version;
	uint16_t length_h;	/* Hack: SPCI Alpha2 splits 32 bit length... */
	uint16_t length_l;	/* ... value over two 16 bit aligned fields. */
	uint16_t attributes;
	uint32_t count;
	uint32_t buf_desc[];
};
struct msg_buf_list_exchange_desc {
	uint16_t flags;
	/* Hack: SPCI spec starts 64 bit pa at this 16 bit aligned offset */
	/* TODO: remove this "reserved" fields added specifically for aarch32 */
	uint16_t reserved[3];
	uint64_t pa;
	uint16_t id;
	TEE_UUID uuid;
			...
			unsigned int count)
{
	...
	for (i = 0; i < count; i++)
		EMSG("Message buffer: id %u, RX %u@%p, TX %u@%p",
			buf->id, buf->rx_size, buf->rx, buf->tx_size, buf->tx);
}
		      ...
		      struct sm_nsec_ctx *nsec __maybe_unused)
{
	...
	assert(fnum >= SPCI_FID_MIN && fnum <= SPCI_FID_MAX);
	...
	return false; /* SMC handled, return to non-secure */
}
...
uint32_t realy_msg_to_vm_rx(struct thread_eret_args *args __unused)
{
	...
	DMSG("vm_id = %u  / from args->a7: %" PRIu32, vm_id, args->a7);
