...
unsigned long __weak plat_scmi_voltd_get_level(unsigned int agent_id __unused,
					       unsigned int scmi_id __unused)
{
	return 0;
}
...
int32_t __weak plat_scmi_voltd_set_level(unsigned int agent_id __unused,
					 unsigned int scmi_id __unused,
					 unsigned long microvolt __unused)
{
	return SCMI_NOT_SUPPORTED;
}
...
static void scmi_voltd_describe_levels(struct scmi_msg *msg)
{
	...
	status = plat_scmi_voltd_levels_array(msg->agent_id, domain_id, NULL, 0,
					     &nb_levels);
	if (status == SCMI_SUCCESS) {
		/* Use the stack to get the returned portion of levels array */
		unsigned long plat_levels[LEVELS_ARRAY_SIZE_MAX];
		size_t ret_nb = 0;
		size_t rem_nb = 0;
		...
		if (status == SCMI_SUCCESS) {
			...
			uint32_t *voltage = NULL;
			size_t i = 0;

			/* By construction these values are 32bit aligned */
			voltage = (uint32_t *)(uintptr_t)(msg->out +
							  sizeof(out_args));
...
static bool message_id_is_supported(size_t message_id)
{
	return message_id < ARRAY_SIZE(scmi_voltd_handler_table) &&
	       scmi_voltd_handler_table[message_id];
}
